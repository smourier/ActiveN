...
{
    private static readonly Lazy<ConcurrentDictionary<int, string[]>> _messagesNames = new(GetMessageNames, true);
    ...
    private static ConcurrentDictionary<int, string[]> GetMessageNames()
    {
        ...
        return new ConcurrentDictionary<int, string[]>(dic.Select(kv => new KeyValuePair<int, string[]>(kv.Key, [.. kv.Value])));
    }

    public static string Decode(this Message msg) => Decode(msg.HWnd, msg.Msg, msg.WParam, msg.LParam);
