...
[GeneratedComClass]
public partial class ConnectionPoint : IConnectionPoint, IDisposable
{
    ...
    public ConnectionPoint(Guid interfaceId)
    {
    ...
    HRESULT IConnectionPoint.Advise(nint sink, out uint cookie)
    {
        ...
        var disp = Com.ComObject.FromPointer<IDispatch>(sink);
        if (disp == null)
        {
            cookie = 0;
        ...
        public HRESULT Next(uint count, CONNECTDATA[] rgcd, out uint fetched)
        {
            ...
            if (fetched > 0)
            {
                for (var i = _index + 1; i < fetched; i++)
                {
                    ...
                    rgcd[i].pUnk = Com.ComObject.GetOrCreateComInstance(connections[i].Value);
